...
FileProgress.prototype.setComplete = function(up, info) {	
    ...
    if (res.url) {
        url = res.url;
        str = "<div><span>" + res.url + "</span><br><input type='hidden' name='images[]' value='" + res.url + "' /><input type='text' name='imagesname[]' value='" + name + "' /><input type='button' class='l-btn l-btn-small' onclick='$(this).parent().parent().parent().parent().parent().remove();' value='删除图片' /></div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div><span>" + link + "</span><br><input type='hidden' name='images[]' value='" + link + "' /><br><input type='button' class='l-btn l-btn-small' style='font-size:12px' onclick='$(this).parents(\"tr:first\").remove();' value='删除图片' /></div>";
    }
    ...
    var showImg = $('<img src="/static/Easyui/themes/default/images/loading.gif"/>');
    ...
    if (!isImg) {
        ...
        $(img).on('load', function() {
            ...
            function initImg(url, key, height) {
                ...
                modalBody.find('img').attr('src', '/static/Easyui/themes/default/images/loading.gif');
