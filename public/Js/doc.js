...
var a, dp = {
    sh: {
        ...
        ClipboardSwf: _ROOT_+"Public/Js/ZeroClipboard.swf",
        Version: "1.5.1"
    }
};
...
var ZeroClipboard = {
    version: "1.0.7",
    clients: {},
    moviePath: _ROOT_+"Public/Js/ZeroClipboard.swf",
    nextId: 1,
    $: function(b) {
        if (typeof(b) == "string") {
...
ZeroClipboard.Client.prototype = {
    ...
    receiveEvent: function(e, f) {
        ...
        if (this.handlers[e]) {
            for (var c = 0,
            b = this.handlers[e].length; c < b; c++) {
...
$(document).ready(function() {
    $(".doc-content pre").each(function() {
        var b = $(this);
        if (b.attr("class") && b.attr("class").indexOf("brush:") != -1) {
...
$(function() {
	...
	$('.nav-user').hover(function() {
		...
		setTimeout(function() {
			...
			$('.nav-user-tab').slideUp('fast');
		},100);
		nav_hover = false;
	})	
	$('.nav-user-tab').hover(function() {nav_hover = true},function() {nav_hover=false});
	...
	$('.doc-pic').hover(function(){
		$(this).append("<div class='slide-text'>缓解视觉疲劳 <a class='set' href='javascript:setDocPic()'>设置</a></div>");
		$(this).find('div').slideDown();
	},function() {
		$(this).find('div').remove();	
	})	
	
	$('.doc-pic-class li').click(function() {
		$(this).toggleClass('selected');	
	})
	$('#set-doc-pic .save-set').click(function() {
		var class_id = [];
		$('.ThinkBox-content .doc-pic-class li.selected').each(function() {
			class_id[class_id.length] = $(this).attr('val');	
		})
		$.post(_ROOT_+'index/setdocpic',{class_id:class_id},function(data) {
			$('.ThinkBox-close').click();
			if(data.status == 1) {
				ThinkSuccess('设置主题成功',1500,function() {window.location.reload();});
			} else {
...
function setDocPic() {
	...
	$.ThinkBox($('#set-doc-pic'),{title:'设置图片主题',drag:true})
}

var loginhtml = false;
function needLogin(reflesh,login_callback) {
	if(loginhtml === false) {
		loginhtml = $('#login-div').html();	
		$('#login-div').remove();
	}
	if(!loginhtml) {
	...
	var html = "<div id='need-login'>"+loginhtml+'</div>';
	$('.ThinkBox-wrapper').remove();
	$.ThinkBox(html,{title:'您需要先登录',drag:true});
	$('#need-login .login_sub').unbind().click(function() {
		var user_name =  $.trim($('.login_left input[name=user_name]').val());
		var password =  $.trim($('.login_left input[name=password]').val());
		if(user_name.length == 0) {
			alert('请输入用户名');
			$('.login_left input[name=user_name]').focus();
			return false;	
		}
		if(password.length == 0) {
			alert('请输入密码');
			$('.login_left input[name=password]').focus();
			return false;	
		}
		$('.login_left input[name=last]').parent().next().html('<font color="green">登录中...</font>');
		var is_last = $('.login_left input[name=last]').val();
		$.post(_ROOT_+'login/ajaxlogin',{user_name:user_name,password:password,last:is_last},function(data) {
			if(data.status == 0) {
				$('.login_left input[name=last]').parent().next().html('<font color="red">'+data.data+"</font>");	
			} else {
				$('.login_left input[name=last]').parent().next().html('<font color="green">登录成功</font>');
				if(reflesh) {
	...
	$('.login_right .register_sub').unbind().click(function() {
		$('.login_right input[name=user_name]').change();
		if(name_check !== true) {
			alert(name_check);
			$('.login_right input[name=user_name]').focus();
			return false;	
		}
		
		$('.login_right input[name=email]').change();
		if(email_check !== true) {
			alert(email_check);
			$('.login_right input[name=email]').focus();
			return false;	
		}
		
		$('.login_right input[name=password]').change();
		if(password_check !== true) {
			alert(password_check);
			$('.login_right input[name=password]').focus();
			return false;	
		}	
		
		var user_name = $('.login_right input[name=user_name]').val();
		var password = $('.login_right input[name=password]').val();
		var email = $('.login_right input[name=email]').val();
		var _this = this;
		$(_this).prev().find('span').html('<font color="green">提交数据中...</font>');
		$.post(_ROOT_+'login/ajaxreg',{user_name:user_name,password:password,email:email},function(data) {
			if(data.status == 0) {
				$(_this).prev().find('span').html('<font color="red">'+data.data+'</font>');	
			} else {
				window.location.reload();
				$('.ThinkBox-close').click();
				ThinkSuccess(data.data,2000);
			}
		},'json')
	})
	
	$('.login_right input[name=user_name]').unbind().change(function() {
		var _this = this;
		$.ajax({
			url:_ROOT_+'login/checkinfo',
			dataType:"json",
			type:'post',
			data:{user_name:$(_this).val()},
			async:false,
			success: function(data) {
				if(data.data.user_name !== true) {
					name_check = data.data.user_name;
					$(_this).next().html('<font color=red>'+name_check+'</font>');	
				} else {
					name_check = true;
					$(_this).next().html('<font color=green>可用</font>');	
				}
			}	
		})	
	})

	$('.login_right input[name=email]').change(function() {
		var _this = this;
		$.ajax({
			url:_ROOT_+'login/checkinfo',
			dataType:"json",
			...
			success: function(data) {
				if(data.data.email !== true) {
					email_check = data.data.email;
					$(_this).next().html('<font color=red>'+email_check+'</font>');	
				} else {
					email_check = true;
					$(_this).next().html('<font color=green>可用</font>');	
				}
	...
	$('.login_right input[name=password]').change(function() {
		var password = $.trim($(this).val());
		if(password.length < 6) {
			password_check = '用户密码过短';
			$(this).next().html('<font color=red>用户密码过短</font>');	
		} else {
			password_check = true;	
			$(this).next().html('<font color=green>可用</font>');	
		}
	})
...
function loadCard() {
	$('#card').remove();
	var obj_card = $('<div id="card"></div>');
	$('body').append(obj_card);
	$('.load-card').unbind().hover(function() {
		$('#card').hide();
		...
		if(pos.left<900) {
			ileft = pos.left+width+10;	
		} else {
			ileft = pos.left-360;	
		}
		itop = pos.top-20;
		$('#card').css({left:ileft,top:itop});
		setTimeout(function() {
			if(!is_hover) {
				return false;	
			}
		$.post(_ROOT_ + 'ajax/getcard',{uid:uid},function(data) {
			if(data.status == 1) {
				$('#card').html(data.data);
				$('#card').show();	
			}
		...
		is_hover = false;
		setTimeout('outCard()',900);	
	})	
	
	$('#card').unbind().hover(function() {is_hover = true;},function() {
		is_hover = false;
		setTimeout('outCard()',900);	
	})
}
...
function ThinkSuccess(msg,time,callback) {
	...
	$('#notice_div #success')[0].play();
}

function needBindEmail() {
	var html = "<p style='padding:20px;'>您需要先绑定邮箱，&nbsp;<a class='btn btn-success' href='"+_ROOT_+"user/edit.html?type=email'>立即绑定</a>";	
	$.ThinkBox(html,{title:'您需要先绑定邮箱',drag:true});
}

function getProvinceList(obj,type,city_id) {
	type = type || 1;
	city_id = city_id || 0;
	var url = _ROOT_+'city/province';
	if(type == 2) {
		url = _ROOT_+'city/city';	
	} else if(type == 3) {
		url = _ROOT_+'city/county';	
	}

	$.get(url,{id:city_id},function(data) {
		if(data.status == 1) {
			obj.children().filter(function() {return $(this).val()>0}).remove();
			for(var i in data.data) {
				obj.append('<option value='+data.data[i]['id']+'>'+data.data[i]['name']+'</option>');	
			}	
		}
	},'json')
}