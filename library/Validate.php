...
class Validate {
			...
			if(isset($v['role'])&&!empty($v['role'])){
				...
				foreach($role as $cv){
					...
					switch(strtolower(trim($cv[0]))){
						case 'required':
							if(strval($v['value'])===''){				
									$result[$v['name']][]	=	$v['msg'].'不能为空';
							}
							break;
						case 'min':
							if($v['value']!==''&&isset($cv[1])&&$cv[1]>0&&strlen($v['value'])<$cv[1]){				
									$result[$v['name']][]	=	$v['msg'].'最小长度不足' . $cv[1] . '位';
							}
							break;
						case 'max':
							if($v['value']!==''&&isset($cv[1])&&$cv[1]>0&&strlen($v['value'])>$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'最大长度不能超过' . $cv[1] . '位';
							}
							break;						
						case 'gt':
							if($v['value']!==''&&isset($cv[1])&&$v['value']<=$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'必须大于' . $cv[1];
							}
							break;
						case 'gte':
							if($v['value']!==''&&isset($cv[1])&&$v['value']<$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'必须大于等于' . $cv[1];
							}
							break;
						case 'lt':
							if($v['value']!==''&&isset($cv[1])&&$v['value']>=$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'必须小于' . $cv[1];
							}
							break;
						case 'lte':
							if($v['value']!==''&&isset($cv[1])&&$v['value']>$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'必须小于等于' . $cv[1];
							}
							break;
						case 'eq':
							if($v['value']!==''&&isset($cv[1])&&$v['value']<>$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'不一致';
							}
							break;
						case 'neq':
							if($v['value']!==''&&isset($cv[1])&&$v['value']==$cv[1]){
									$result[$v['name']][]	=	$v['msg'].'必须不等于' . $cv[1];
							}
							break;
						case 'in':
							$in = explode(',',strtolower($cv[1]));
							if($v['value']!==''&&!in_array(strtolower($v['value']), $in)){
									$result[$v['name']][]	=	$v['msg'].'必须包含在[' . $cv[1] . ']';
							}
							break;
						case 'like':
							if($v['value']!==''&&stripos($v['value'], $cv[1])===FALSE){
									$result[$v['name']][]	=	$v['msg'].'必须相似于%' . $cv[1] . '%';
							}
							break;
						case 'between':
							$between = explode(',',$cv[1]);
							if($v['value']!==''&&isset($between[0])&&($v['value']<$between[0]||$v['value']>$between[1])){
									$result[$v['name']][]	=	$v['msg'].'必须间于' . $between[0].'-'.$between[1];
							}
							break;
						case 'unique':
							$table = explode('.',$cv[1]);
							if($v['value']!==''&&$table[0]!==''){
									...
									if( DB::table($table[0])->where($column, '=', $v['value'])->count()>0 ){
										$result[$v['name']][]	=	$v['msg'].'已存在';
									}
							}
							#role='unique:table_members.phone';检测值将来会在表中是唯一的,即当前不存在
							break;
						case 'exists':
							$table = explode('.',$cv[1]);
							if($v['value']!==''&&$table[0]!==''){
									$column = empty($table[1]) ? $v['name'] : $table[1];
									if( DB::table($table[0])->where($column, '=', $v['value'])->count()<1 ){
										$result[$v['name']][]	=	$v['msg'].'不存在';
									}
							}
							#role='exists:table_members.phone';检测值在表中已存在
							break;	
					}
				}}
			}
	...
	public static function isUsername($value,$minLen=2,$maxLen=32){
		if(preg_match('/^[_\.\w\d\x{4e00}-\x{9fa5}]{'.$minLen.','.$maxLen.'}$/ius',$value)){
			...
			return	['code'=>0, 'msg'=>'格式有误,请输入2-30位的字符'];
		}
	}
